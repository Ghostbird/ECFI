...
secretfunction:
	...
	bl	puts	@
	mov	r0, #0	@,
        LDR	R0, =0 @if hotsiteid is less than 1024 use MOV instead.
        MOV	R1, LR
...
vuln:
	...
	sub	sp, fp, #4	@,,
	@ sp needed	@
        LDR	R0, =1 @if hotsiteid is less than 1024 use MOV instead.
        MOV	R1, LR
        LDR     r3, =rb_write_attached
        PUSH    {LR}
        BLX     r3
...
main:
	...
	@ frame_needed = 1, uses_anonymous_args = 0
        ...
        LDR	r0, =72168  @ Main address in CFG injected here.
